...
local duration_old = (get_time_usec() - start_old)
...
local duration_new = (get_time_usec() - start_new)

ngx.say(string.format("Old way (os.date x2): %.2f us", duration_old))
ngx.say(string.format("New way (ngx.utctime + sub): %.2f us", duration_new))
